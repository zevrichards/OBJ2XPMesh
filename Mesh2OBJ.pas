...
begin
  ...
  S := DSFTXTEdit.text+'.elevation.raw';

  FS:= TFileStream.Create(S, fmopenread + fmShareDenyNone{ or fmShareDenyWrite});
  total_rows := trunc(sqrt(FS.Size/8)*2);  //total_rows=total_columns

  try
    FS.Position:=0;
    SetLength(RAW_Array, total_rows, total_rows);

    For r := 0 to total_rows-1 do
    begin
      For c := 0 to total_rows-1 do
      begin
        FS.Seek(r*c, soFromCurrent);
        FS.Read(b, 2);
        b := Swap(b);
        RAW_Array[r,c] := b;
      end;
...
//    Determine row and column position based on fraction of total rows/columns
    ...
    row := trunc(((Extents[1]-(aVertex.Y))/(Extents[1]-Extents[2]))*total_rows);      //(N - vertex_lat)/(N-S) * total_rows
    column := trunc(((Extents[3]-(aVertex.X))/(Extents[3]-Extents[4]))*total_rows);    //(W - vertex_long)/(W-E) * total_columns

    aVertex.z := RAW_array[row, column];

    If aVertex.z > 0 then
      Memo1.Lines.add('X: '+IntToStr(Trunc(Row))+#9+'Y: '+IntToStr(Trunc(Column))+#9+'Lat: '+FloatToStr(aVertex.y)+#9+'Long: '+FloatToStr(aVertex.x)+#9+'Height: '+FloatToStr(b));
  end;
...
//  result.z:= StrToFloat(trim(value));
  ...
  If result.lat > Extents[1] then
    Extents[1] := result.lat;
