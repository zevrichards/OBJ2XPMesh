  ...
  begin
    ...
    begin
      ...
      1:begin //triangle fans
          While v < aPrimitive.children.count do
            begin
              //create faces from last 3 verts added, first in an anti-clockwise then clockwise direction
...
begin
    ...
    begin
      ...
      begin
        ...
        repeat
          inc(y)
        until DSF_SL.Strings[y]='END_PATCH';
...
begin
  ...
  If ((ContainsText(OBJ_SL.Text, 'vt ')) and (ContainsText(OBJ_SL.Text, 'vn '))) then   //if the object containts both vt and vn indexes then the v indexes will be at locations 1,4 and 7
  begin
    v2:= 4;
    v3:= 7;
  end
  ...
  begin
    If ((ContainsText(OBJ_SL.Text, 'vt ')) or (ContainsText(OBJ_SL.Text, 'vn '))) then   //if only vt, or only vn then it will be at 1,3 and 5
    begin
      v2:= 3;
      v3:= 5;
    end
    else             //if only v then 1,2 and 3.
    begin
      v2:= 2;
      v3:= 3;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          aGroup.ter_def := s[g+1];     //the terrain definition should immediatlely follow the Mesh ID
          break;
        end;
      end;
      anOBJ.children.Add(aGroup);
...
begin
  ...
  begin
    ...
    begin
      ...
      if aVertex = nil then
        aVertex := anOBJ.FindVertex(aFace.v3);  //only search the rest of the OBJ if the face is not found in this group... much slower
      ...
      if aVertex = nil then
        aVertex := anOBJ.FindVertex(aFace.v2); //only search the rest of the OBJ if the face is not found in this group... much slower
      DSF_SL.Insert(x,'PATCH_VERTEX'+#9+FloatToStrF(aVertex.long/100000, ffFixed, 13, 9)+#9+FloatToStrF(aVertex.lat/100000, ffFixed, 13, 9)+#9+FloatToStrF(aVertex.height, ffFixed, 13, 9)+#9+'-0.000015259'+#9+'-0.000015259'+#9+'1.000000000'+#9+'1.000000000');
      inc(x);

      aVertex := aGroup.FindVertex(aFace.v1);
      if aVertex = nil then
        aVertex := anOBJ.FindVertex(aFace.v1); //only search the rest of the OBJ if the face is not found in this group... much slower
      DSF_SL.Insert(x,'PATCH_VERTEX'+#9+FloatToStrF(aVertex.long/100000, ffFixed, 13, 9)+#9+FloatToStrF(aVertex.lat/100000, ffFixed, 13, 9)+#9+FloatToStrF(aVertex.height, ffFixed, 13, 9)+#9+'-0.000015259'+#9+'-0.000015259'+#9+'1.000000000'+#9+'1.000000000');
      inc(x);
    end;
...
procedure TForm1.RemoveDuplicateFaces;    //this should only be used when covering the ENTIRE Mesh with photo-scenery as XP11 seems to use duplicate faces for smooth transitions between terrain types
...
begin
  //determine whether a face shares identical vertices with another
  result := false;
...
begin
  //determine whether verts share the same space
  result := false;
...
var state, match: boolean;
    ...
    OBJ_SL: TStringList;
begin
  //splits 1 face into 4 by adding 3 more verts at the midpoint of each line
  face_count := 0;
  vert_count := 0;
  prim_count := 0;
