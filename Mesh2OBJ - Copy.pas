...
type TPATCH = class
  ...
  near_LOD: integer;
  far_LOD: integer;
  flag: integer;      //type of mesh: 1 - physics/collision; 2 - overlay
  num_point_coords: integer;
  children: TList<TPRIMITIVE>;
  constructor create(s: String);
end;

type TDSF = class
  children: TList<TPATCH>;
end;

type TOBJ_Vertex = class
  x,y,z: real;
  constructor Create(aPatchVertex: TPATCH_VERTEX); override;
...
var
  Form1: TForm1;
  patch_count, prim_count, vert_count, RAW_rows: integer;
...
begin
  ...
  self.near_LOD := StrToInt(s.Split(Delimiters)[2]);
  self.far_LOD := StrToInt(s.Split(Delimiters)[3]);
...
constructor TPATCH_VERTEX.create(s: string);
begin

  inc(vert_count);

  self.id:= vert_count;
  self.long := StrToFloat(s.Split(Delimiters)[1]);
  self.lat := StrToFloat(s.Split(Delimiters)[2]);
  self.height := Trunc(StrToFloat(trim(s.Split(Delimiters)[3])));
...
//    Open DSF.txt file
    ...
    begin
      ...
      If ContainsText(CurrentLine, 'BEGIN_PATCH') then
        aDSF.children.Add(aPatch.create(CurrentLine));
      If ContainsText(CurrentLine, 'BEGIN_PRIMITIVE') then
        aPatch.children.add(aPrimitive.create(CurrentLine));
      If ContainsText(CurrentLine, 'PATCH_VERTEX') then
      begin
        aPrimitive.children.add(aPatch_Vertex.create(CurrentLine));
        SetExtents(aPatch_Vertex);  //determine extents of mesh coverage
      end;
    end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        aPatchVertex := TOBJ_VERTEX.create(CurrentLine);
    ...
    repeat                                           //collect all verts
      ...
      begin
        ...
        aPatchVertex.long := StrToFloat(trim(CurrentLine.Split(Delimiters)[lon]));
        aPatchVertex.lat := StrToFloat(trim(CurrentLine.Split(Delimiters)[lat]));
        aPatchVertex.height := StrToInt(trim(CurrentLine.Split(Delimiters)[height]));
        If lat = 3 then
          aPatchVertex.lat := -aPatchVertex.lat;
...
procedure TForm1.SetExtents(aPatchVertex: TPATCH_VERTEX);
begin
    //northern most vertex
  If aPatchVertex.lat > Extents[1] then
    Extents[1] := aPatchVertex.lat;
  //southern most vertex
  If aPatchVertex.lat < Extents[2] then
    Extents[2] := aPatchVertex.lat;
  //western most vertex
  If aPatchVertex.long < Extents[3] then
    Extents[3] := aPatchVertex.long;
  //eastern most vertex
  If aPatchVertex.long > Extents[4] then
    Extents[4] := aPatchVertex.long;
end;
